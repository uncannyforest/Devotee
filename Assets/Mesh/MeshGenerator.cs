...
public class MeshGenerator : MonoBehaviour {
    ...
    private void AddCorner(List<Vector3> verts, int i) {
        ...
        if (!HasGap(i, middle)) {
            ...
            verts.Add(Rot(i) * new Vector3(0, 0, 1)); // outer base                 c
        } else {
            ...
            verts.Add(Rot(i) * new Vector3(0, corners[i] / scale, 1)); // outer rim     c - 2 top
            verts.Add(Rot(i) * new Vector3(0, corners[i] / scale, 1)); // outer rim     c - 1 outer
            MarkVertex(verts, upNormals);
            verts.Add(Rot(i) * new Vector3(0, 0, 1)); // outer base                     c     outer
        }
    }
    ...
    private void AddSide(List<Vector3> verts, List<int> tris, int i0, int c0, int c1) {
        ...
        if (!gap0 && !gap1) {
            ...
            tris.AddRange(Tri(gap0, c2, c2 - 1, c3)); // outer face
