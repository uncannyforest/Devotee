...
public class FreeLookCamMovement : MonoBehaviour
{
    ...
    [Range(0f, 10f)] [SerializeField] private float m_HTurnSpeed = 3f;  // How fast the rig will rotate left-right from user input.
    [Range(0f, 1f)] [SerializeField] private float m_VTurnSpeed = .02f;   // How fast the rig will rotate up-down and forward-back from user input.
    ...
    [SerializeField] private AnimationCurve m_DistanceCurve = new AnimationCurve(
            ...
            new Keyframe(3f, 4), 
            new Keyframe(5, 12f));
    [SerializeField] private float m_DistanceCurveConstantFactor = 1f; // rather than changing the whole curve . . .
    [SerializeField] private float m_DefaultTiltDistanceFrame = 3f;
    ...
    {
        if (Input.GetButtonDown("Reset Camera") && m_AutoScript.IsDoubleResetReady) {
